...
@section('title')
Jaya Motor | Edit Form Pembelian
@endsection
...
<script>
    ...
    form.action = "{{ route('editpembelianproses') }}"; // Ganti dengan route untuk proses
    form.submit();
    });
    });
...
<form id="formPembelian" action="{{ route('editpembelianproses') }}" method="post">
    @csrf
    <div class="grid gap-4 mb-4 grid-cols-2">
        ...
        <div class="col-span-1">
            ...
            <input type="date" name="tanggal_pembelian" id="tanggal_pembelian"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                value="{{ $dataPembelian -> tanggal_pembelian }}" required="">
        </div>
        <div class="col-span-1">
            ...
            <select name="kode_supplier" id="kode_supplier"
                ...
                <option value="{{ $dataPembelian -> kode_supplier }}" selected="">Pilih Supplier</option>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mb-5">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            ...
            <tbody>
                @foreach ($dataPembelianDt as $itemPembelianDt)
                <tr class="bg-white border-b">
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                        {{ $itemPembelianDt -> nama_barang }}
                    </th>
                    <td class="px-6 py-4">
                        {{ $itemPembelianDt -> jumlah_barang }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $itemPembelianDt -> satuan_barang }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $itemPembelianDt -> deskripsi_barang }}
                    </td>
                    <td class="px-6 py-4">
                        <span class="currency">{{ $itemPembelianDt -> harga_satuan_barang }}</span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="currency">{{ $itemPembelianDt -> harga_total_barang_sbd }}</span>
                    </td>
                    <td class="px-6 py-4">
                        {{ $itemPembelianDt -> persen_diskon_barang }}
                    </td>
                    <td class="px-6 py-4">
                        <span class="currency">{{ $itemPembelianDt -> rupiah_diskon_barang }}</span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="currency">{{ $itemPembelianDt -> harga_total_barang_std }}</span>
                    </td>
                    <td class="px-6 py-4">
                        <!-- Modal toggle -->
                        <button data-modal-target="EditBarangModal" data-modal-toggle="EditBarangModal"
                            data-kode_pembdetil="{{ $itemPembelianDt ->kode_pembdetil }}" data-kode_pembelian="{{
                        $itemPembelianDt -> kode_pembelian }}" data-nama_barang="{{ $itemPembelianDt -> nama_barang }}"
                            data-jumlah_barang="{{ $itemPembelianDt -> jumlah_barang }}" data-satuan_barang="{{
                        $itemPembelianDt -> satuan_barang }}" data-deskripsi_barang="{{ $itemPembelianDt ->
                        deskripsi_barang }}" data-harga_satuan_barang="{{ $itemPembelianDt -> harga_satuan_barang }}"
                            data-harga_total_barang_sbd="{{ $itemPembelianDt -> harga_total_barang_sbd }}"
                            data-persen_diskon_barang="{{ $itemPembelianDt -> persen_diskon_barang }}"
                            data-rupiah_diskon_barang="{{ $itemPembelianDt -> rupiah_diskon_barang }}"
                            data-harga_total_barang_std="{{ $itemPembelianDt -> harga_total_barang_std }}" class=" edit text-green-700 hover:text-white border border-green-700 hover:bg-green-800
                        ...
                        <button data-modal-target="popup-modal" data-modal-toggle="popup-modal"
                            data-kode_pembdetil="{{ $itemPembelianDt ->kode_pembdetil }}"
            ...
            <div class="flex flex-col">
                <div class="col-span-1 flex flex-row mb-2">
                    ...
                    <input type="text" name="persen_diskon_pembelian" id="persen_diskon_pembelian"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder=" 20" value="{{ $dataPembelian -> persen_diskon_pembelian }}">
                </div>
                <div class="flex flex-row ">
                    <label class="my-auto w-2/5" for="">Rp.</label>
                    <input type="number" name="rupiah_diskon_pembelian" id="rupiah_diskon_pembelian"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder=" 100.000" value="{{ $dataPembelian -> rupiah_diskon_pembelian }}">
                </div>
            </div>
        </div>
        <div class="col-span-2 px-5 border py-5 rounded-lg xl:mb-0 mb-5">
            ...
            <div class="flex flex-col">
                <div class="col-span-1 flex flex-row mb-2">
                    ...
                    <input type="text" name="persen_ppn_pembelian" id="persen_ppn_pembelian"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder=" 20" value="{{ $dataPembelian -> persen_ppn_pembelian }}">
                </div>
                <div class="flex flex-row ">
                    <label class="my-auto w-2/5" for="">Rp.</label>
                    <input type="number" name="rupiah_ppn_pembelian" id="rupiah_ppn_pembelian"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder=" 100.000" value="{{ $dataPembelian -> persen_ppn_pembelian }}">
                </div>
            </div>
        </div>
        <div class="col-span-2 px-5 border py-5 rounded-lg xl:mb-0 mb-5">
            ...
            <div class="flex flex-col">
                <div class="col-span-1 flex flex-row mb-2">
                    ...
                    <input type="text" name="persen_biayalain_pembelian" id="persen_biayalain_pembelian"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder=" 20" value="{{ $dataPembelian -> persen_biayalain_pembelian }}">
                </div>
                <div class="flex flex-row ">
                    <label class="my-auto w-2/5" for="">Rp.</label>
                    <input type="number" name="rupiah_biayalain_pembelian" id="rupiah_biayalain_pembelian"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder=" 100.000" value="{{ $dataPembelian -> persen_biayalain_pembelian }}">
                </div>
            </div>
        </div>
        <div class="col-span-2 px-5 border py-5 rounded-lg xl:mb-0 mb-5">
            ...
            <div class="flex flex-col">
                ...
                <div class="col-span-1 flex flex-row mb-2">
                    ...
                    <input type="number" name="pembulatan_pembelian" id="pembulatan_pembelian"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder=" 1" value="{{ $dataPembelian -> pembulatan_pembelian }}">
                </div>
                <div class="col-span-1 flex flex-row mb-2">
                ...
                <div class="flex flex-row mb-2">
                    ...
                    <input type="number" name="jumlah_bayar_pembelian" id="jumlah_bayar_pembelian"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                        placeholder=" 100.000" required value="{{ $dataPembelian -> jumlah_bayar_pembelian }}">
                </div>
                ...
                <div class="flex flex-row mb-10">
                    ...
                    <textarea name="deskripsi_pembelian" id="deskripsi_pembelian" rows="4"
                        ...
                        maxlength="255">{{ $dataPembelian -> deskripsi_pembelian }}</textarea>
                </div>
                <div class="flex flex-row justify-between">
                    <button type="button" id="prosesBtn"
                        class="mx-2 w-full block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                        Proses
                    </button>
                </div>
            </div>
...
<div id="AddBarangModal" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <!-- Modal body -->
            <form id="AddDataBarang" action="{{ route('editpembelianbrginsert') }}" method="POST" class="p-4 md:p-5">
                @csrf
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        <label for="kode_pembelian" class="block mb-2 text-sm font-medium text-gray-900">ID Pembelian
...
<div id="EditBarangModal" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <!-- Modal body -->
            <form id="EditDataBarang" action="{{ route('editpembelianbrgupdate') }}" method="POST" class="p-4 md:p-5">
                @csrf
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="p-4 md:p-5 text-center">
                ...
                <form id="delete-form"
                    action="{{ route('editpembelianbrgdelete', ['kode_pembdetil' => 'PLACEHOLDER']) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
